...
-- Checks if Specialist availability impacts overall wait time and shows stage breakdown
SELECT 
    ...
    -- Calculate what percentage of total wait is spent at each stage
    AVG(Time_to_Registration_min) * 1.0 / NULLIF(AVG(Total_Wait_Time_min), 0) * 100 AS registration_percentage,
    AVG(Time_to_Triage_min) * 1.0 / NULLIF(AVG(Total_Wait_Time_min), 0) * 100 AS triage_percentage,
    AVG(Time_to_Medical_Professional_min) * 1.0 / NULLIF(AVG(Total_Wait_Time_min), 0) * 100 AS doc_time_percentage
FROM dbo.ER_Wait_Times
